...
import { buildFilterHref, type Paginated, type ProductFilters } from "@vortexis/commerce/client";
...
export function Pagination({
  ...
  return (
    <nav className="mt-14 flex items-center justify-center gap-1.5" aria-label="Paginação">
      <PageLink
        href={buildFilterHref(filters, { pagina: page - 1 }, basePath)}
      ...
      {pages.map((entry, index) =>
        entry === "…" ? (
          ...
          <PageLink
            key={entry}
            href={buildFilterHref(filters, { pagina: entry }, basePath)}
            current={entry === page}
            label={`Página ${entry}`}
          >
            {entry}
...
function PageLink({
  ...
  const className = cn(
    ...
    current
      ? "bg-caramel-deep text-warm-white"
